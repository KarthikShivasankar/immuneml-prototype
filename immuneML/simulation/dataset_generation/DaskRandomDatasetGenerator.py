...
class RandomDatasetGenerator:
    ...
    def _check_labels(labels: dict):
        if labels is not None:
            ...
            for label in labels:
                RandomDatasetGenerator._check_probabilities(
                    labels[label], object, f"labels - {label}")
    ...
    def _check_rep_dataset_generation_params(repertoire_count: int, sequence_count_probabilities: dict, sequence_length_probabilities: dict,
        ...
        RandomDatasetGenerator._check_example_count(
            repertoire_count, "repertoire_count")
        RandomDatasetGenerator._check_probabilities(
            sequence_count_probabilities, int, "sequence_count_probabilities")
        RandomDatasetGenerator._check_probabilities(
            sequence_length_probabilities, int, "sequence_length_probabilities")
    ...
    def generate_repertoire_dataset(repertoire_count: int, sequence_count_probabilities: dict, sequence_length_probabilities: dict,
        ...
        if labels is not None:
            processed_labels = {label: random.choices(list(labels[label].keys(
            )), labels[label].values(), k=repertoire_count) for label in labels}
            dataset_params = {label: list(
                labels[label].keys()) for label in labels}
        else:
        ...
        dataset = RepertoireDataset(
            labels=dataset_params, repertoires=repertoires, metadata_file=metadata)
    ...
    def _check_receptor_dataset_generation_params(receptor_count: int, chain_1_length_probabilities: dict,
        ...
        RandomDatasetGenerator._check_probabilities(
            chain_1_length_probabilities, int, 'chain_1_length_probabilities')
        RandomDatasetGenerator._check_probabilities(
            chain_2_length_probabilities, int, 'chain_2_length_probabilities')
        RandomDatasetGenerator._check_example_count(
    ...
    def generate_receptor_dataset(receptor_count: int, chain_1_length_probabilities: dict, chain_2_length_probabilities: dict, labels: dict,
        ...
        def get_random_sequence(proba, chain, id): return ReceptorSequence("".join(random.choices(alphabet, k=random.choices(list(proba.keys()),
                                                                                                                             proba.values())[0])),
        ...
        receptors = [TCABReceptor(alpha=get_random_sequence(chain_1_length_probabilities, "TRA", i),
                                  beta=get_random_sequence(
                                      chain_2_length_probabilities, "TRB", i),
        ...
        receptor_matrix = np.core.records.fromrecords([receptor.get_record(
        ) for receptor in receptors], names=TCABReceptor.get_record_names())
    ...
    def _check_sequence_dataset_generation_params(receptor_count: int, length_probabilities: dict, labels: dict, path: Path):
        RandomDatasetGenerator._check_probabilities(
            length_probabilities, int, 'length_probabilities')
        RandomDatasetGenerator._check_example_count(
            receptor_count, "receptor_count")
        RandomDatasetGenerator._check_labels(labels)
        RandomDatasetGenerator._check_path(path)
    ...
    def generate_sequence_dataset(sequence_count: int, length_probabilities: dict, labels: dict, path: Path):
        ...
        sequence_matrix = np.core.records.fromrecords(
            [seq.get_record() for seq in sequences], names=ReceptorSequence.get_record_names())
