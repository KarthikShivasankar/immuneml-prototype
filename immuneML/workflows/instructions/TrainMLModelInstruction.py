...
class TrainMLModelInstruction(Instruction):
    ...
    def run(self, result_path: Path):
        ...
        self.state.report_results = HPUtil.run_hyperparameter_reports(
            self.state, self.state.path / "reports")
    ...
    def _compute_optimal_hp_item_per_label(self):
        ...
        for idx, label in enumerate(self.state.label_configuration.get_label_objects()):
            self._compute_optimal_item(
                label, f"(label {idx + 1} / {n_labels})")
            zip_path = MLExporter.export_zip(
                hp_item=self.state.optimal_hp_items[label.name], path=self.state.path / f"optimal_{label.name}", label_name=label.name)
            self.state.optimal_hp_item_paths[label.name] = zip_path

    def _compute_optimal_item(self, label: Label, index_repr: str):
        optimal_hp_settings = [
            state.label_states[label.name].optimal_hp_setting for state in self.state.assessment_states]
        ...
        if self.state.refit_optimal_model:
            ...
            optimal_assessment_state = self.state.assessment_states[optimal_hp_settings.index(
                optimal_hp_setting)]
    ...
    def print_performances(self, state: TrainMLModelState):
        print(
            f"Performances ({state.optimization_metric.name.lower()}) -----------------------------------------------", flush=True)

        for label_name in state.label_configuration.get_labels_by_name():
            print(f"\n\nLabel: {label_name}", flush=True)
            print(
                f"Performance ({state.optimization_metric.name.lower()}) per assessment split:", flush=True)
            for split in range(state.assessment.split_count):
                print(
                    f"Split {split+1}: {state.assessment_states[split].label_states[label_name].optimal_assessment_item.performance[state.optimization_metric.name.lower()]}", flush=True)
    ...
    def _export_optimal_performances_to_csv(self):
        for label_name in self.state.label_configuration.get_labels_by_name():
            ...
            pd.DataFrame(performance).to_csv(
                self.state.path / f"{label_name}_optimal_models_performance.csv", index=False)
        ...
        for label_name in self.state.label_configuration.get_labels_by_name():
            ...
            for index, assessment_state in enumerate(self.state.assessment_states):
                for hp_setting, hp_item in assessment_state.label_states[label_name].assessment_items.items():
                    ...
                    performance['split'].append(index+1)
                    performance['optimal'].append(
                        hp_setting == assessment_state.label_states[label_name].optimal_hp_setting)
                    for metric in self.state.metrics:
                        performance[metric.name.lower()].append(
                            hp_item.performance[metric.name.lower()])
            pd.DataFrame(performance).to_csv(
                self.state.path / f"{label_name}_all_assessment_performances.csv", index=False)
                    ...
                    for i, hp_item in enumerate(hp_item_list):
                        ...
                        for metric in self.state.metrics:
                            performance[metric.name.lower()].append(
                                hp_item.performance[metric.name.lower()])
                pd.DataFrame(performance).to_csv(
                    self.state.path / f"{label_name}_assessment_split_{index+1}_selection_performance.csv", index=False)
