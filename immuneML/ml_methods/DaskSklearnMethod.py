...
class DaskSklearnMethod(MLMethod):
    ...
    def _fit_by_cross_validation(self, X, y, number_of_splits: int = 5, label: Label = None, cores_for_training: int = 1,
        ...
        if self.client == None:
            ...
            self.model = RandomizedSearchCV(model, param_distributions=self._parameter_grid, cv=number_of_splits, n_jobs=-1,
                                            scoring=scoring, refit=True)
    ...
    def store(self, path: Path, feature_names=None, details_path: Path = None):
        ...
        file_path_hmac = path / \
            f"{self._generate_hmac_digest()}-{self._get_model_filename()}.pickle"

        with file_path.open("wb") as file:
            dill.dump(self.model, file)

        with file_path_hmac.open("wb") as hmac_file:
            dill.dump(self.model, hmac_file)
    ...
    def _generate_hmac_digest(self):

        model = dill.dumps(self.model)

        self.digest = hmac.new(str.encode(os.environ.get(
            'secret')), model, hashlib.blake2b).hexdigest()
    ...
    def load(self, path: Path, details_path: Path = None):
        ...
        hmac_name = f"{self._generate_hmac_digest()}-{self._get_model_filename()}.pickle"
        digest = hmac_name.split('-')[0]

        hmac_file_path = path / hmac_name
        if hmac_file_path.is_file():
            with open(hmac_file_path, "rb") as hmacbin:
                unpicked_bytes = hmacbin.read()

            expected_digest = hmac.new(str.encode(os.environ.get(
                'secret')), unpicked_bytes, hashlib.blake2b).hexdigest()

            if not secrets.compare_digest(digest, expected_digest):
                raise SystemExit("Invalid Signature")
                exit(1)
            else:
                with open(hmac_file_path, "rb") as securefile:

                    self.model = dill.load(securefile)
