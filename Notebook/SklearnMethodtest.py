...
class SklearnMethodtest(MLMethod):
    """
    ...
    def convert_to_onnx(self, training_data: EncodedData, path: Path):
        ...
        if self.model is not None:
            ...
            onx = to_onnx(self.model, training_data[:3])
    ...
    def load(self, path: Path, details_path: Path = None):
        ...
        if hmac_file_path.is_file():
            with open(hmac_file_path, "rb") as file:
                unpicked_bytes = file.read()
